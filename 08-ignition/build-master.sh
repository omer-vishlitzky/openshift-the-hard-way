#!/bin/bash
# Build master ignition config
# Masters fetch their real config from the Machine Config Server
...
OUTPUT_FILE="${IGNITION_DIR}/master.ign"
...
cat > "${OUTPUT_FILE}" <<EOF
{
  "ignition": {
    ...
    "config": {
      "merge": [
        {
          "source": "https://api-int.${CLUSTER_DOMAIN}:22623/config/master"
        }
      ]
    },
    "security": {
...
if jq . "${OUTPUT_FILE}" > /dev/null 2>&1; then
    echo "Master ignition built successfully"
    echo "Output: ${OUTPUT_FILE}"
    echo "Size: $(du -h "${OUTPUT_FILE}" | cut -f1)"
    echo ""
    echo "This ignition will fetch config from:"
    echo "  https://api-int.${CLUSTER_DOMAIN}:22623/config/master"
else
